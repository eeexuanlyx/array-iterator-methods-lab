...
const between1500and1599 = (inventors) =>
  inventors.year > 1500 && inventors.year < 1599;

veryOldInventors.push(inventors.filter(between1500and1599));
...
const newArray = inventors.map((name) => ({
  first: name.first,
  last: name.last,
}));
inventorNames.push(newArray);
...
newSorted = inventors.sort((a, b) => a.year - b.year);
sortedByBirthYear.push(newSorted);
...
foundAda = inventors.find((person) => person.first === "Ada");
inventorNamedAda = foundAda;
...
function formattedNames(element) {
  const parts = element.split(", ");
  return `${parts[1]} ${parts[0]}`;
}

firstLast = people.map(formattedNames);
...
const greaterThan18 = (year) => year > "2006";
isAdultPresent = devs.some(greaterThan18);
...
const atLeast19 = (year) => year >= "2005";
isEveryone19OrOlder = devs.some(atLeast19);
...
commentById = comments.find((item) => item.id === 823423);
...
function find123523(value) {
  return value.id === 123523;
}

idx = comments.findIndex(find123523);
...
totalYearsLived = inventors.reduce((acc, curVal) => {
  const yearsLived = curVal.passed - curVal.year;
...
travelMethodCounts = travelMethods.reduce((acc, travelmethod) => {
  acc[travelmethod] = (acc[travelmethod] || 0) + 1;
  return acc;
}, {});
